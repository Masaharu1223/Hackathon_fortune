...
exports.DEFAULT_SEED_REGION = exports.seedStoreRegions = void 0;
const minamiMachidaStores = [
    {
...
function buildMeguroKujiSeries(storeNumber, slotNumber) {
    const template = meguroKujiTemplates[(storeNumber + slotNumber - 2) % meguroKujiTemplates.length];
    const variation = ((storeNumber * 7) + (slotNumber * 3)) % 13 - 6;
    const remainingTickets = template.status === 'sold_out'
        ? 0
        : template.status === 'upcoming'
            ? template.totalTickets
            : Math.max(1, Math.min(template.totalTickets - 1, template.remainingTickets + variation));
    return {
        ...template,
        seriesId: `seed-meguro-station-series-${String(storeNumber).padStart(3, '0')}-${String(slotNumber).padStart(2, '0')}`,
        remainingTickets,
    };
}
// Generate a denser cluster around Meguro Station for nearby-store testing.
const meguroStationStores = meguroStoreBlueprints.map((blueprint, index) => {
    ...
    return {
        ...
        kujiSeries: Array.from({ length: blueprint.seriesCount }, (_, slotIndex) => buildMeguroKujiSeries(storeNumber, slotIndex + 1)),
    };
});
exports.seedStoreRegions = {
    'minami-machida': {
        label: '南町田周辺',
...
exports.DEFAULT_SEED_REGION = 'meguro-station';
//# sourceMappingURL=devStoreRegions.js.map